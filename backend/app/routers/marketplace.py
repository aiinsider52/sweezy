...
def delete_listing(listing_id: str, db: DBSession, user: CurrentUser) -> None:
    ...
    if listing.author_id != user.id and not is_admin:
...
@admin_router.get("/marketplace", response_model=list[ServiceListingDetail])
...
@admin_router.patch("/marketplace/{listing_id}/approve", response_model=ServiceListingDetail)
...
@admin_router.patch("/marketplace/{listing_id}/reject", response_model=ServiceListingDetail)
