...
_CANTON_TERMS = {
    "ZH": ("zurich", "zürich", "winterthur"),
    "BE": ("bern", "berne", "biel", "bienne"),
    "LU": ("lucerne", "luzern"),
    "ZG": ("zug",),
    "BS": ("basel",),
    "BL": ("liestal", "basel-landschaft"),
    "AG": ("aargau", "aarau", "baden"),
    "SG": ("st. gallen", "st gallen", "rapperswil"),
    "GR": ("graubünden", "grisons", "chur", "davos"),
    "TI": ("ticino", "lugano", "bellinzona", "locarno"),
    "VD": ("vaud", "lausanne", "montreux"),
    "GE": ("geneva", "genève", "genf"),
    "FR": ("fribourg", "freiburg"),
    "NE": ("neuchâtel", "neuchatel"),
...
def _infer_workplace(text: str) -> str | None:
    low = text.lower()
    if any(
        term in low for term in ("remote", "home office", "homeoffice", "télétravail")
    ):
        return "remote"
...
def _infer_requirements(text: str) -> tuple[list[str], bool, bool, bool]:
    ...
    permits = [
        code
        for code in ("B", "C", "S", "L", "G")
        if re.search(rf"(?:permit|bewilligung|permis)\s*{code}\b", text, re.IGNORECASE)
    ]
    no_experience = any(
        term in low
        for term in (
...
def _parse_salary(value: str | None) -> tuple[int | None, int | None, str | None]:
    ...
    period = (
        "year"
        if any(term in low for term in ("year", "jahr", "annum"))
        else (
            "hour"
            if any(term in low for term in ("hour", "stunde", "/h"))
            else ("month" if any(term in low for term in ("month", "monat")) else None)
        )
    )
...
def _csv_env(name: str) -> list[str]:
    return [
        value.strip() for value in (os.getenv(name) or "").split(",") if value.strip()
    ]
...
async def _fetch_jooble(client: httpx.AsyncClient) -> list[NormalizedJob]:
    ...
    base_url = (
        (os.getenv("JOOBLE_API_BASE_URL") or "https://de.jooble.org/api")
        .strip()
        .rstrip("/")
    )
    if not base_url.startswith("https://"):
        raise RuntimeError("JOOBLE_API_BASE_URL must use HTTPS")
...
async def _fetch_greenhouse(client: httpx.AsyncClient) -> list[NormalizedJob]:
    ...
    for board in boards:
        ...
        for row in response.json().get("jobs", []):
            ...
            company = (
                _clean_text(row.get("company_name"), 250)
            ...
            output.append(
                NormalizedJob(
                    ...
                    degree_required=degree,
                    recognition_required=recognition,
                    posted_at=_parse_date(row.get("updated_at")),
    ...
    for site in sites:
        ...
        for row in response.json():
            ...
            description = _clean_text(
                " ".join(
                    str(row.get(k) or "")
                    for k in ("description", "descriptionPlain", "additional")
                ),
                30_000,
            )
            ...
            salary_text = None
            if (
                salary_range.get("min") is not None
                or salary_range.get("max") is not None
            ):
...
async def _fetch_personio(client: httpx.AsyncClient) -> list[NormalizedJob]:
    ...
    for company in companies:
        response = await client.get(
            f"https://{company}.jobs.personio.de/xml", params={"language": language}
        )
        response.raise_for_status()
        ...
        for row in root.findall(".//position"):
            ...
            description = _clean_text(
                " ".join((child.text or "") for child in row.findall(".//*")), 30_000
            )
            if not title:
                continue
            permits, no_experience, degree, recognition = _infer_requirements(
            ...
            output.append(
                NormalizedJob(
                    source=f"personio:{company}",
                    source_job_id=_source_id("personio", raw_id, url, title, company),
                    title=title,
...
_PROVIDERS = {
    ...
    "greenhouse": (
        lambda: bool(_csv_env("GREENHOUSE_BOARD_TOKENS")),
        _fetch_greenhouse,
    ),
...
def _upsert_jobs(
    db: Session, rows: list[NormalizedJob], now: datetime
) -> tuple[int, list[Job]]:
    changed: list[Job] = []
    for item in rows:
...
async def translate_job_description(
    db: Session, job: Job, language: str
) -> tuple[str, bool]:
...
def _matches_alert(job: Job, alert: JobAlert) -> bool:
    ...
    words = [
        word for word in re.split(r"[,\s]+", alert.keywords.lower()) if len(word) > 1
    ]
    return (
        ...
        and (
            ...
            or alert.employment_type.lower() in (job.employment_type or "").lower()
        )
        and (not alert.workplace_type or alert.workplace_type == job.workplace_type)
        and (
            alert.min_salary is None
            or (job.salary_max or job.salary_min or 0) >= alert.min_salary
        )
    )
...
def _enqueue_alerts(db: Session, jobs: list[Job], now: datetime) -> int:
    if not jobs or not settings.PUSH_NOTIFICATIONS_ENABLED:
        return 0
    alerts = (
        db.execute(select(JobAlert).where(JobAlert.enabled.is_(True))).scalars().all()
    )
    count = 0
    for alert in alerts:
        ...
        for job in matches:
            ...
            payload = {
                "aps": {
                    "alert": {"title": alert.name, "body": job.title},
                    "sound": "default",
                },
                "type": "job_alert",
                "job_id": job.id,
...
async def sync_jobs(
    db: Session, provider_names: list[str] | None = None
) -> dict[str, Any]:
...
def provider_health(
    db: Session, *, include_errors: bool = False
) -> list[ProviderHealth]:
    rows = {
        row.provider: row
        for row in db.execute(select(JobProviderState)).scalars().all()
    }
    ...
    for name, (configured, _) in _PROVIDERS.items():
        ...
        output.append(
            ProviderHealth(
                ...
                status=row.status
                if row
                else ("pending" if is_configured else "disabled"),
                last_success_at=row.last_success_at if row else None,
                last_item_count=row.last_item_count if row else 0,
                message=(row.last_error if include_errors else "Provider sync failed")
...
def job_to_item(job: Job, now: datetime | None = None) -> JobItem:
    ...
    freshness = (
        ...
        else ("recent" if age <= timedelta(hours=72) else "stale")
    )
    return JobItem(
        id=job.id,
        source=job.source,
...
def search_catalog(
    ...
    *,
        ...
        for term in terms:
            ...
            conditions.append(
                or_(
                    ...
                    Job.skills.cast(String).ilike(like),
                )
            )
    if canton:
    ...
    if min_salary is not None:
        conditions.append(
            or_(Job.salary_max >= min_salary, Job.salary_min >= min_salary)
        )
    stmt = select(Job).where(and_(*conditions))
    total = (
        db.scalar(select(func.count()).select_from(Job).where(and_(*conditions))) or 0
    )
    rows = (
        db.execute(
            stmt.order_by(
                Job.is_promoted.desc(),
                Job.is_verified.desc(),
    ...
    stale = bool(rows) and all(
        job_to_item(row, now).freshness == "stale" for row in rows
    )
    return (
        [job_to_item(row, now) for row in rows],
        int(total),
        {str(k): int(v) for k, v in sources.items()},
        stale,
    )
...
def _tokens(value: str) -> set[str]:
    ...
    return {
        word for word in re.findall(r"[\w+#.-]{2,}", value.lower()) if word not in stop
    }


def _explainable_score(
    profile: JobMatchProfile, job: Job
) -> tuple[int, list[str], list[str]]:
    desired = _tokens(profile.desired_position)
    skills = {skill.strip().lower() for skill in profile.skills if skill.strip()}
    job_text = _tokens(
        f"{job.title} {job.description or ''} {' '.join(job.skills or [])}"
    )
    reasons: list[str] = []
    missing: list[str] = []
    score = 20
    ...
    skill_matches = {
        skill
        for skill in skills
        if skill in (job.description or "").lower() or skill in job_text
    }
    if skill_matches:
    ...
    if (
        ...
        and profile.employment_type.lower() in (job.employment_type or "").lower()
    ):
        score += 5
    ...
    if profile.permit and (
        not job.permit_requirements
        or profile.permit.upper() in {p.upper() for p in job.permit_requirements}
    ):
        score += 5
...
async def match_jobs(db: Session, profile: JobMatchProfile) -> JobMatchResponse:
    ...
    if (
        ...
        and rows
        and (profile.desired_position or profile.skills)
    ):
        try:
            from openai import AsyncOpenAI
            ...
            texts = [query] + [
                f"{job.title}. {job.company or ''}. {job.snippet or job.description or ''}"[
                    :6000
                ]
                for job in rows
            ]
            ...
            for index, vector in enumerate(vectors[1:]):
                ...
                norm = (
                    math.sqrt(
                        sum(a * a for a in query_vector) * sum(b * b for b in vector)
                    )
                    or 1
                )
                ...
                scores[index] = (
                    min(100, round(base * 0.45 + semantic * 100 * 0.55)),
                    reasons,
                    missing,
                )
            method = "semantic"
    ...
    ranked = sorted(zip(rows, scores), key=lambda pair: pair[1][0], reverse=True)[
        : profile.limit
    ]
    quality_fields = [
    ...
    quality = round(
        sum(bool(value) for value in quality_fields) / len(quality_fields) * 100
    )
    return JobMatchResponse(
        items=[
            ...
            for job, (score, reasons, missing) in ranked
        ],
        method=method,
...
async def search_jobs(
    q: str | None, canton: str | None, page: int, per_page: int, debug: bool = False
):
